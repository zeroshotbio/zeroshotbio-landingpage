...
- **3D protein–molecule viewer** (Step 1, 2D⇄3D toggle): lazy-loads 3Dmol.js from CDN (no package.json
  change). Every drug has a precomputed RDKit ETKDG **3D conformer**; 10/16 also have a **verified
  target-complex** (RCSB-checked PDB), e.g. rapamycin at the FKBP12–mTOR interface (1FAP), bortezomib in
  the 20S proteasome (5LF3), lapatinib–EGFR (1XKK), paclitaxel–β-tubulin (1JFF). Ligand highlighted,
  protein as spectrum cartoon; drag/zoom. Honest labels (conformer geometry is illustrative; complexes
  are real experimental structures).
