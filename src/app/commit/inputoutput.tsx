...
// ── atoms ──────────────────────────────────────────────────────────────────
function RowLabel({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <div style={{ display: "flex", gap: 9, alignItems: "baseline", marginBottom: 7 }}>
      <span style={{ fontFamily: MONO, fontSize: 9, fontWeight: 700, color: FAINT, minWidth: 11 }}>
        {n}
      </span>
...
function ColHead({ side, title, sub }: { side: string; title: string; sub: string }) {
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 650, color: INK, marginTop: 5, letterSpacing: -0.2 }}>{title}</div>
      <div style={{ fontSize: 12.5, color: MUTED, marginTop: 3, lineHeight: 1.5 }}>{sub}</div>
    </div>
  );
}
  ...
  .cio-arrow{transform:rotate(90deg)}
      ...
      <div className="cio-grid" style={{ background: CARD, border: `1px solid ${RULE}`, borderRadius: 12, overflow: "hidden" }}>
        ...
        <div className="cio-col cio-col--in">
          <ColHead side="input" title="What you are handed"
                   sub={`Per cluster. Shown for ${EXAMPLE_CLUSTER}.`} />

          <Row n={1} label="three ranked gene lists">
            {EX ? (
              <>
                <SubItem tag="enriched" note="top_50_markers">
                  <Chips list={EX.top_50_markers} show={4} />
                </SubItem>
                <SubItem tag="depleted" note="bottom_50_markers">
                  <Chips list={EX.bottom_50_markers} show={4} />
                </SubItem>
                <SubItem tag="family" note="shared with related clusters">
                  <Chips list={EX.family_50_markers} show={4} />
                </SubItem>
              </>
            ) : (
              <div style={{ fontSize: 12.5, color: MUTED }}>enriched · depleted · family — 50 genes each, in rank order</div>
            )}
          </Row>
          ...
          <Row n={3} label="cluster identity">
            <div style={{ display: "flex", gap: 10, alignItems: "baseline", flexWrap: "wrap" }}>
              <span style={{ fontFamily: MONO, fontSize: 14, fontWeight: 700, color: INK }}>{EX?.cluster_id ?? "Cxxx"}</span>
              <span style={{ fontFamily: MONO, fontSize: 12.5, color: MUTED, fontVariantNumeric: "tabular-nums" }}>
                {EX ? `${nfmt(EX.n_cells)} cells` : "n_cells"}
              </span>
            </div>
            <div style={{ fontSize: 11.5, color: FAINT, marginTop: 5, lineHeight: 1.5 }}>
              The id is opaque — it carries no ordering and no meaning beyond identity.
            </div>
          </Row>

          <Row n={4} label="the full expression matrix">
            <div style={{ fontFamily: MONO, fontSize: 14, fontWeight: 700, color: INK, fontVariantNumeric: "tabular-nums" }}>
              {matrix}
            </div>
            <div style={{ fontSize: 11.5, color: FAINT, marginTop: 5, lineHeight: 1.5 }}>
              Raw counts and log1p CP10k, for every cluster at once — there if you would rather
              compute your own evidence than take ours.
            </div>
          </Row>

          {/* the beat that makes the column mean something */}
          <div style={{ marginTop: 16, padding: "13px 15px", background: "#faf9f7", border: `1px solid ${RULE}`, borderRadius: 9 }}>
            <div style={{ fontFamily: MONO, fontSize: 9, fontWeight: 700, letterSpacing: 0.7, textTransform: "uppercase", color: "#9a3b3b", marginBottom: 8 }}>
              not given
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "5px 8px" }}>
              {["ZSCAPE's four label columns", "its published marker calls", "the excluded-cluster names", "the key"].map((t) => (
                <span key={t} style={{ fontSize: 11.5, color: FAINT, textDecoration: "line-through", textDecorationColor: "#d8cfc9" }}>
                  {t}
                </span>
              )).reduce((acc: React.ReactNode[], el, i) => {
                if (i > 0) acc.push(<span key={`s${i}`} style={{ color: "#ddd8d1", fontSize: 11 }}>·</span>);
                acc.push(el);
                return acc;
              }, [])}
            </div>
        ...
        <div className="cio-col">
          <ColHead side="output" title="What you must return"
                   sub="Per cluster. One answer, four parts." />
