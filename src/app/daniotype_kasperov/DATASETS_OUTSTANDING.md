...
| ~~MIC-Drop~~ | ~~Hash `micdrop_2-6-25.rds` against the GEO object~~ | ~~differ by 8.6 MB~~ | **DONE 2026-08-13** — both hashed and byte-compared: identical for the first **23,355,451,955** uncompressed bytes, diverging only in a ~28.9 MB trailing-slot tail. Same data; either source yields the same H5AD |
...
| ~~MIC-Drop~~ | ~~Cell Ranger version~~ | ~~GEO says v7; "v5.0.0" appears in no released artifact~~ | **CLOSED 2026-08-13 without a browser.** `cellranger-5.0.0` is machine-written into the `web_summary.html` of samples x1–x4 in the Zenodo bundle we already held (x2–x4 were renamed `50_x*.html`, which is why an earlier grep missed them). **GEO's "v7" is a hand-entered error.** Same files give `Include introns: False` — the matrix is exon-only, with 9.1–9.5% of confidently-mapped reads discarded as intronic. Pilot version still unknown (no summary survives) |
| MIC-Drop | Paper's 135,881 genotyped cells not reproducible | **Reframed 2026-08-13.** "50 genes × avg 2,718/genotype" pins the denominator: the count excludes `tyr`, so the right comparison is our **134,518**, not 138,767. The deposit is **1,363 short (1.0%)** — so **no filter can explain it**, filters only remove. **scDblFinder is refuted**: it appears only in the pilot's `Figure1` Rmd, explicitly to *validate* that `Multiple` cells are doublets, and `Figure2` contains none | Open — genuinely unresolved; searched full text, Supp. Data 1–5, Source Data, Zenodo bundle and all four Rmds |
