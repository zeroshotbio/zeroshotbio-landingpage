...
export const REASONER_PROMPT = [
  ...
  "",
  "GUT-CHECK BEFORE YOU CONCLUDE (literature-grounded): once you have a first de-novo call in mind, list your key cross-tier assumptions — the lineage/umbrella chain (e.g. ectoderm ⊃ epidermis ⊃ keratinocyte) and the core-biology notions behind it — then dispatch the Researcher ONCE to VERIFY them in the literature (PREFIX that dispatch with 'WIDE LIT REVIEW:' to unlock its broad-literature mode beyond ZFIN/ZFA/GO), returning a citation AND a short supporting quote per assumption. Conclude only once the assumptions are literature-checked, with confidence reflecting the gut-check.",
