...
const DATASETS: DatasetDef[] = [
  ...
  {
    // ZSCAPE Commit Gold — a distinct benchmark row (own atlasId, own run history; never merged
    // with ZSCAPE Classic). Clustering is GIVEN/frozen (ZSCAPE's published 112 clusters), so its
    // runs are not leaf-comparable to Classic's ~250 recursive leaves. Assets served under
    // zscape_commit_gold/ (built when the first run is minted — see the row's LEDGER #1: not blind).
    id: "zscape_commit_gold",
    name: "ZSCAPE Commit Gold",
    tagline: "ZSCAPE 48 hpf control arm · 112 published clusters (≥50 cells) · ZFA-menu-constrained · gold key held out",
    blurb:
...
const DATASET_VALIDATION: Record<string, string> = {
  zscape_commit_gold: "Benchmark cut · scaffolded · no runs yet · ⚠ NOT BLIND — we hold the answer key on-instance (LEDGER #1)",
...
const DATASET_ORDER: DatasetId[] = ["zscape_commit_gold", "zscape", "chemfish", "daniocell", "megafin_parse", "megafin", "minifin"];
