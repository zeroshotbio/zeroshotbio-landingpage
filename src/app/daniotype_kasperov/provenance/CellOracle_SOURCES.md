...
> **No annotation is distributed.** GSE145298 contains matrices only. The cell-type labels used in
> the paper cannot be recovered from the deposit.
...
> **What is NOT distributed:** the fitted GRNs themselves (TF–target edge tables), the CellOracle
> simulation vectors, and the differentiation vector fields. The paper points to
> <https://celloracle.org> for interactive exploration; no downloadable network or simulation object
> is deposited in GEO or with the paper. **UNRESOLVED.**
...
- **No cell-type annotations exist in the deposit.** Using this dataset for anything label-dependent
  requires re-deriving labels ourselves, or obtaining them from the authors. This is the dominant
  limitation.
...
- **No GRNs, TF–target edge tables, simulation vectors or vector fields are distributed** despite
  being the paper's central output.
