...
GT-blind, then names each leaf from control cells only.
...
3. **Naming** — control cells (`gene_target` startswith `ctrl-`) vote each leaf's tissue name; the
   all-cells vote is kept alongside. The partition is GT-blind; names are a provisional scaffold.

## Params that matter (verified on disk)
COARSE_RES 0.1 · MIN_COMP 500 · N_HVG 2000 (seurat) · SCALE_MAX 10 · N_PCS 50 · N_NEIGHBORS 15 ·
LOCAL_RES 0.8 · SEED 7 · vote = control-only primary, all-cells secondary. Served golden = 250 leaves.
