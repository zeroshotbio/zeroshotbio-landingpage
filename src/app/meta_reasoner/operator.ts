  ...
  const system = [
    ...
    input.scope === "compartment"
      ? [
          ...
          "- NODE_LABEL PHRASING (name each node cleanly): give each merged node ONE clean canonical anatomical/cell-type term — NO 'X / Y' slash-compounds, NO parenthetical qualifiers, NO verbose descriptive coining. Prefer the SPECIFIC canonical term the members share ('periderm', not 'superficial epidermal keratinocyte'; 'retinal pigmented epithelium', not 'pigment epithelium'). You MAY coin a clean shared abstraction when the members are a mix under one identity ('myeloid leukocyte' for macrophage+neutrophil members) or reuse a member label — whichever is the cleanest correct shared identity.",
