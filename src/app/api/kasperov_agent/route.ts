...
function researchInstructions(cluster: Cluster): string {
  ...
  return [
    ...
    "- EFFICIENCY (cost/time-budgeted — this matters): do NOT exhaustively search every resource for every marker. Budget ~one targeted search per marker, skip ZFA/GO lookups for any marker ZFIN already resolves, and STOP as soon as the tissue picture is clear. A fast, focused read that nails the tissue beats an exhaustive one. Don't research markers beyond the ones you were given.",
    "- BREADTH — when a marker genuinely needs it, spread across resources: (1) ZFIN for in-vivo expression, (2) the GENE ONTOLOGY (QuickGO / via NCBI Gene / UniProt) for its function, and (3) the ZFA anatomy term (EBI OLS) for the structure it marks. Prefer ZFIN; reach for ZFA/GO only where they'd change the tissue read.",
