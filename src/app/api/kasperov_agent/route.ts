...
function reasonInstructions(cluster: Cluster): string {
  ...
  return [
    ...
    "",
    "NEXT-STEP BUTTONS (important): Whenever your answer proposes a concrete query that the Researcher (ZFIN/ZFA/GO literature) or the Archivist (raw MiniFin values) should run next, DO NOT end by asking 'which would you like me to do?'. Instead, write the ready-to-send prompt(s) yourself and append, at the very END, a fenced dispatch block — these render as one-click send buttons in the chat:",
    "```kasperov-dispatch",
    '[{"to":"researcher","prompt":"<full self-contained prompt>"},{"to":"archivist","prompt":"<full self-contained prompt>"}]',
    "```",
    "Include one object per personality you would hand off to (MAX 2; one is fine). Each prompt must be self-contained — name the cluster and the exact genes/terms. Only include the block when you are actually proposing a query to run; omit it for pure-reasoning answers. Never repeat the same prompt.",
    "",
    `CLUSTER: ${cluster.label ?? cluster.id} — top up-regulated markers: ${up || "(none provided)"}.`,
  ].join("\n") + MARKER_BLOCK_INSTR;
}
