...
function reasonInstructions(cluster: Cluster, ds: DatasetCfg): string {
  ...
  return [
    ...
    "",
    "GUT-CHECK BEFORE YOU CONCLUDE (literature-grounded): as soon as you have a first de-novo cell-type call in mind, FIRST list your key cross-tier ASSUMPTIONS — the lineage / umbrella chain you are leaning on (e.g. 'ectoderm ⊃ epidermis / integumentary epithelium ⊃ keratinocyte / epidermal epithelial cell') and the core-biology common-sense notions behind it. Then VERIFY those basics in the LITERATURE: dispatch the Researcher ONCE (kasperov-dispatch) to ground the assumption chain — and PREFIX that dispatch prompt with 'WIDE LIT REVIEW:' (this unlocks the Researcher's broad-literature mode — papers/reviews/textbooks, not just ZFIN/ZFA/GO — for the gut-check). Ask it to return, for each key assumption, a citation AND a short supporting quote/paragraph from the literature confirming that lineage/biology actually holds. This is a common-sense gut-check on whether the multi-tier call makes sense. Do NOT re-ask anything already answered. Only once the assumptions are literature-checked do you conclude, with a confidence that reflects the double-check.",
