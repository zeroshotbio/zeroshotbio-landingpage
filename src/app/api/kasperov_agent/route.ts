...
// Appended to the Researcher's system prompt ONLY when the Reasoner invokes a
// "WIDE LIT REVIEW:" dispatch — lifts the ZFIN/ZFA/GO domain restriction for a
// broad-literature gut-check of the Reasoner's lineage/biology assumptions.
const WIDE_RESEARCH_ADDENDUM =
  "\n\n=== WIDE LIT REVIEW MODE (Reasoner-invoked gut-check) ===\n" +
  "For THIS request ONLY, your web search is NOT restricted to ZFIN/ZFA/GO — search the broad scientific literature (peer-reviewed papers, reviews, textbooks, PubMed/NCBI, developmental-biology sources). PURPOSE: ground the Reasoner's core-biology / lineage ASSUMPTIONS as a common-sense gut-check (e.g. 'is ectoderm the umbrella of epidermis/keratinocyte?'). " +
  "For each assumption, return a citation LINK and a short supporting quote/paragraph from the source that confirms or challenges it. Still link every claim; never fabricate. This is a reasoning sanity-check — it does NOT change this cluster's per-marker evidence.";
...
function researchInstructions(cluster: Cluster): string {
  ...
  return [
    ...
    "- EFFICIENCY (bounded, not skipped): ~one targeted ZFIN web search per marker, and exactly ONE ontology_lookup(zfa) + ONE ontology_lookup(go) per marker — these are fast, deterministic tool calls, so ALWAYS make them; do not loop or exhaustively web-search, and don't research markers beyond the ones you were given. Bounded-but-complete beats fast-but-thin: the goal is all three sources per marker, done efficiently — never dropped.",
    "- BREADTH (required for EVERY marker): report all three sources — (1) ZFIN in-vivo expression (web search), (2) the GO molecular-function/process term via ontology_lookup(go), and (3) the ZFA anatomy term via ontology_lookup(zfa). ZFA and GO come ONLY from the ontology_lookup tool (web search does not reliably surface them). Do NOT skip ZFA/GO for a marker just because ZFIN resolved it — anatomy + function coverage must be consistent across the whole report.",
...
function reasonInstructions(cluster: Cluster, ds: DatasetCfg): string {
  ...
  return [
    ...
    "",
    "GUT-CHECK (ONLY when the lineage is genuinely non-obvious): for textbook-clear cell types (epidermis, skeletal muscle, blood, neurons, and the like) SKIP the gut-check and conclude directly — do not spend a Researcher turn confirming the obvious. ONLY when the cross-tier lineage is genuinely uncertain, or you are weighing competing identities, dispatch the Researcher ONCE (kasperov-dispatch) — PREFIX that dispatch with 'WIDE LIT REVIEW:' (unlocks broad-literature mode) — to verify the shaky assumption(s), returning a citation + short supporting quote. Do NOT re-ask anything already answered. This is a targeted tie-breaker, not a routine step.",
    ...
    "```kasperov-conclude",
    '{"identity":"<deepest defensible zebrafish cell-type or anatomy name>","tier":"<germ layer|tissue|cell type>","state":"<progenitor|cycling|quiescent|mature|stress|none>","cited_markers":["<gene from THIS cluster\'s marker list>"],"decision":"<assign|abstain>","confidence":<0-100>,"stack":{"germ_layer":{"prediction":"<label>","confidence":<0-100>},"tissue":{"prediction":"<label>","confidence":<0-100>},"cell_type_broad":{"prediction":"<label>","confidence":<0-100>},"cell_type_sub":{"prediction":"<label>","confidence":<0-100>}},"done":true}',
    "```",
    "The `stack` is REQUIRED and carries the SAME four-tier breakdown you declared in prose above — one short `prediction` label + a `confidence` (0-100) per tier. It populates the four-tier confidence panel directly, so make each prediction your genuine best call at that tier and each confidence honest (they should generally fall from germ layer → sub-type). The top-level `identity`/`tier`/`confidence` stay the deepest tier you actually assign, and must match that line of the stack.",
    ...
    "MENU-AWARE BINNING (second pass, only when asked): AFTER your de-novo conclusion, you may be handed the dataset's published label MENU and asked to fit. Your de-novo call is then FINAL — do not revise it. Produce a SECOND, menu-aware read: for each tier pick the single closest existing MENU option and give it its own confidence. Report BOTH reads clearly (de-novo stack AND menu-aware binning), so we can compare open-vocabulary vs constrained. Then REFLECT on any divergence: for each tier where the menu bin differs from your de-novo call, say whether it is just a menu-vocabulary artifact (the menu lacks your finer/better term) or a real uncertainty — and when it's real, lower that tier's confidence and flag that more research/thinking would help before trusting the bin. Once you are IN the menu-exposed phase, make it explicit you are on the menu-exposed read (not the de-novo one). END the cluster with a clear '✅ CLUSTER COMPLETE' summary that restates BOTH final answers side by side — a 'De-novo:' line and a 'Menu-exposed:' line, each with all four tiers + confidences — so it is unambiguous the cluster is finished. Never let the menu contaminate or overwrite the de-novo answer.",
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // WIDE LIT REVIEW: a Reasoner-invoked gut-check that lifts the domain filter.
      const wideResearch = mode === "research" && /\bWIDE LIT REVIEW\b/i.test(lastUser);
      const instructions = mode === "archivist" ? archivistInstructions(cluster, ds) : mode === "reason" ? reasonInstructions(cluster, ds) : researchInstructions(cluster) + (wideResearch ? WIDE_RESEARCH_ADDENDUM : "");
      const tools = mode === "research"
        ? [wideResearch ? { type: "web_search" } : { type: "web_search", filters: { allowed_domains: ALLOWED_DOMAINS } }, ONTOLOGY_TOOL]
        : mode === "archivist" ? [QUERY_TOOL] : undefined;
