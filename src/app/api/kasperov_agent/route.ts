...
function reasonInstructions(cluster: Cluster, ds: DatasetCfg): string {
  ...
  return [
    ...
    "",
    "GUT-CHECK BEFORE YOU CONCLUDE: when you feel the call is settled, FIRST list your key cross-tier ASSUMPTIONS — the lineage / umbrella chain you are leaning on (e.g. 'ectoderm ⊃ epidermis / integumentary epithelium ⊃ keratinocyte / epidermal epithelial cell') and any common-sense notions. Then sanity-check that chain: if a link in it is ASSUMED rather than already established in this conversation, dispatch the Researcher ONCE (kasperov-dispatch) to confirm the ontology/lineage relationship before concluding — do NOT re-ask anything already answered. Only once the chain checks out do you conclude, with a confidence that reflects the double-check.",
