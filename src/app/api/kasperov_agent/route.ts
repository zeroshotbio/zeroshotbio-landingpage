...
function reasonInstructions(cluster: Cluster, ds: DatasetCfg): string {
  ...
  return [
    ...
    "GESTALT OVER A LONE TISSUE TAG: when the identity must be chosen among markers that are ALL already enriched in this cluster (grounding still rules — a marker must be enriched here to count), do NOT let one marker's curated tissue tag override a lineage that is coherent across the OTHER enriched markers and their functional annotations. Curated tissue tags are frequently incomplete (a gene tagged for one organ is often also expressed in others). If an enriched marker's curated tag conflicts with the functional gestalt of the other enriched markers, FLAG the conflict but weigh it against the whole — never anchor the final call on the single conflicting tag. This is NOT 'literature over stats': every marker you weigh must still be enriched here.",
