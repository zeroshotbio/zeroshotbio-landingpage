...
function archivistInstructions(cluster: Cluster): string {
  return [
    ...
    "USE THE RIGHT KIND: mean / average normalised expression for genes in this cluster → kind='specificity' (it returns activeMean + activePct + rank) or kind='across' (per-cluster means for one gene). Adjusted p-values → kind='pvalues'. So mean expression AND p-values ARE available — fetch them; do not defer or say they're missing.",
