// src/app/api/fqpipe_edits/route.ts
//
// The saved state of the /FASTQ_pipe map — position nudges and
...
//
// SEPARATE ITEM_ID FROM /pipeline AND FROM /bioinformatics_pipe, AND THAT IS
// THE WHOLE POINT OF THIS FILE existing. The three maps have their own object
// sets and their own ids; one record shared between them would mean whichever
// page saved last erased the others' layout, silently, with no way to tell
// which had happened. /bioinformatics_pipe is the closest neighbour and the
// easiest to collide with — it is the other half of the same row.
//
...
const ITEM_ID = "FASTQ_pipe::edits";
const MAX_BYTES = 320_000; // DynamoDB caps an item at 400KB

// GET /api/fqpipe_edits → { offsets, at } or nulls if nothing saved
export async function GET() {
  try {
    ...
    console.error("fqpipe_edits GET failed", err);
...
export async function POST(request: NextRequest) {
  ...
  try {
    await ddbClient.send(
      new PutItemCommand({
        TableName: TABLE_NAME,
        Item: marshall({ id: ITEM_ID, state_json, updated_at, kind: "fqpipe_map_edits" }),
      })
    );
  } catch (err) {
    console.error("fqpipe_edits POST failed", err);
