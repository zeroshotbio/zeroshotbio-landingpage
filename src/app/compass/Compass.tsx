...
export default function Compass({ fontClass }: { fontClass: string }) {
  ...
  return (
    <div className={fontClass} style={{ position: "fixed", inset: 0, background: "#0e1116", color: "#c7d0da", display: "grid", gridTemplateColumns: "1fr 1px 1fr", overflow: "hidden" }}>
      ...
      <Streams cond="wildtype" title="Wild-type development" subtitle={`ChemFish vehicle embryos, n = ${DL.n_embryos.wildtype["48"]} per landmark · a flotilla of ten`} />
      <div style={{ background: "#232b36" }} />
      <Streams cond={drug} title="Drug-treated development" subtitle={`${drug} · ${PATHWAY[drug]} · same delta, n = ${DL.n_embryos[drug]?.["48"]} per landmark`}>
