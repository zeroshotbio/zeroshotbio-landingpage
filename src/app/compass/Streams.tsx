...
const L = 120, XK = [18, 40, 76, 118], WK = [28, 62, 88, 112];                 // knots: tissue fork (schematic), 36, 48, 72 hpf
...
function build() {
  ...
  XK.forEach((_, ki) => {
    ...
    const vis = torder.map((t) => leaves.filter((l) => l.tissue === t && l.fork <= stage));
    const nVis = vis.reduce((a, v) => a + v.length, 0); const gap = Math.max(1.0, (W - nVis * SP) / Math.max(1, torder.length - 1));
    let y = -W / 2;
    vis.forEach((ls, ti) => { ls.forEach((l, i) => { l.y[ki] = y + (i + 0.5) * SP + (hash(l.id + ki) - 0.5) * 0.25; }); y += ls.length * SP + gap * (0.6 + 0.8 * hash(torder[ti] + ki)); });
  ...
  for (const l of leaves) {
    ...
    for (let k = 1; k < XK.length; k++) {
      ...
      else if (k === ki && l !== tr) { const fx = XK[k] - 5 - hash(l.id) * 9; const f = (fx - XK[k - 1]) / (XK[k] - XK[k - 1]); pts.push([fx, tr.y[k - 1] + (tr.y[k] - tr.y[k - 1]) * f], [XK[k], l.y[k]]); }
...
export default function Streams({ cond, title, subtitle, children }: { cond: string; title: string; subtitle: string; children?: React.ReactNode }) {
  ...
  const top = useMemo(() => { const f = (c: string) => Object.fromEntries(S.map((s) => [s, new Set(leaves.filter((l) => l.fork <= s).sort((a, b) => (b.share[c]?.[String(s)] ?? 0) - (a.share[c]?.[String(s)] ?? 0)).slice(0, TOPN).map((l) => l.id))])); return { wt: f("wildtype"), cond: f(cond) }; }, [leaves, cond]);
  const status = (l: Stream, s: number) => { const w = top.wt[s].has(l.id), d = top.cond[s].has(l.id); return isWT ? (w ? "taken" : "open") : d && !w ? "new" : d ? "taken" : "open"; };
  const colorOf = (st: string) => (st === "taken" ? BLUE : st === "new" ? ORANGE : GREY);
  const tissueTaken = (tissue: string) => leaves.some((l) => l.tissue === tissue && top.cond[36].has(l.id));
  ...
  const streams = useMemo(() => (
    <g>
      ...
      {tissues.map((tn) => { const tr = leaves.find((q) => q.tissue === tn && q.trunk); if (!tr) return null; const tk = tissueTaken(tn); const sp = slicePts(tr.pts, 8, XK[1]);
        return <path key={tn} d={centerPath(sp.map((p) => p[0]), sp.map((p) => p[1]))} stroke={tk ? BLUE : GREY} strokeWidth={tk ? 1.4 : 0.5} fill="none" strokeLinecap="round" />; })}
      ...
      {leaves.flatMap((l) => S.filter((s, i) => status(l, s) !== "open" && (i === 0 || status(l, S[i - 1]) === "open")).map((s) => { const ki = XK.indexOf(XLM[s]); const [cx, cy] = iso(XLM[s], l.y[ki], 0.2); return <circle key={`${l.id}-${s}`} cx={cx} cy={cy} r={0.55} fill="#0e1116" stroke={colorOf(status(l, s))} strokeWidth={0.35} />; }))}
    </g>
    ...
    <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
      ...
      <svg viewBox={`${minX} ${minY} ${maxX - minX} ${maxY - minY}`} preserveAspectRatio="xMidYMid meet" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        ...
        {rows.map((x, ri) => {
          ...
          if (x < XK[1]) return tissues.filter(tissueTaken).slice(0, TOPN).map((tn, i) => { const l = leaves.find((q) => q.tissue === tn && q.trunk)!; return <Boat key={`${ri}-${i}`} x={x} y={yAt(l.pts, x)} z={0.15 * Math.sin(t * 2 + i)} color={BLUE} />; });
          const s = stageAt(x);
          return leaves.filter((l) => top.cond[s].has(l.id)).map((l, i) => <Boat key={`${ri}-${l.id}`} x={x} y={yAt(l.pts, x)} z={0.15 * Math.sin(t * 2 + i)} color={colorOf(status(l, s))} />);
        })}
