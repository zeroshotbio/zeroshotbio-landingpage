...
const nextConfig = {
  ...
  // /pipeline is the published map-only SVG viewer; /pipeline_edit keeps the full
  ...
  async rewrites() {
    return {
      ...
      // monotone DECREASING in density, so their sum is, and the plate's only
      // ordering claim holds. Plus a ChemFish drug layer, scroll-zoom and pan.
      ...
      // The drug layer DEFORMS the surface by log2(drug occupancy / vehicle
      // occupancy) along the axis, at 36 and 48 hpf and nowhere else. That is a
      // change in OCCUPANCY, not in position — ChemFish cells reach the axis by
      // which state they are, never by their own coordinates, so a drug cannot
      // move a basin sideways here. The shared-response tint in the valleys is
