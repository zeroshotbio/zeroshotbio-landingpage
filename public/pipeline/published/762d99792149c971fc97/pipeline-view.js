...
const EARLY_OFF=(()=>{
  if(MAP.state) return MAP.state.offsets||{};
  try{
...
NODES.filter(n=>n.shape==="attritionstaircase").forEach(r=>{
  ...
  // _px/_py already include the saved offsets, so subtracting them here
  // always gave zero on load. Apply the stored displacement to the geometry.
  // positionOrigin retains the drawing's pre-fix published position instead
  // of suddenly applying the old, previously invisible accumulated offsets.
  const off=LIVE[r.id]||{}, origin=r.positionOrigin||{};
  const mx=(off.dx||0)-(origin.dx||0), my=(off.dy||0)-(origin.dy||0);
...
if(dotCtx){
  svg.addEventListener("click",ev=>{
    if(editing || MAP.presentation) return;
...
/* ZOOMED OUT, THE MAP IS A STILL PICTURE (2026-09-16, on request).
   ...
   tenth of a pixel. */
// Public map: start at a 30% lower zoom scale than the editor's 0.60 gate.
const MOTION_MIN=MAP.presentation ? 0.42 : 0.60;
...
window.addEventListener("pointermove",e=>{
  ...
  if(pointers.size>=2 && pinch){
    const now=localMid();
    const nk=Math.max(MAP.minZoom??.15,Math.min(MAP.maxZoom??5, pinch.k*(now.d/Math.max(1,pinch.d))));
...
svg.addEventListener("wheel",e=>{
  ...
  const nk=Math.max(MAP.minZoom??.15,Math.min(MAP.maxZoom??5,view.k*Math.exp(-e.deltaY*.0016)));
...
function feature(name, fn){
  // A published scene uses this drawing engine without authoring or polling.
