/* ============================================================
   culls/culls-draw.js — THE FOUR ROOFED CULLS, drawn once for two maps.

   These four shapes were written for /bioinformatics_pipe and are now what
   /pipeline draws between its unfiltered and filtered matrices as well. They
   live here rather than in either page's own shapes file for the reason the
   node prose is lifted rather than re-typed: two hand-maintained accounts of
   one pipeline drift, and the drift is invisible until somebody quotes the
   wrong one. There is one knee chart, and both maps show it.

   WHAT THE HOST PAGE MUST ALREADY HAVE DEFINED. This file is a classic
   script and reads these out of the shared global scope:

     P  el  pts  S  rng  paint       the projection and its primitives
     TICKERS                         the single frame loop's registry
     DRAW                            the shape registry it registers into
     SKIN                            needs .works and .tile
     roofFrame                       from the page's own iso file
     MODEL                           from culls-pop.js, loaded before this

   Both pages' iso files are the same file with different comments, so all of
   the above mean the same thing in both. The one that is NOT identical is
   topOf(): /pipeline returns n.h*0.96 for a `works` node and n.h for
   everything else. These shapes therefore must NOT be given shape:"works" —
   they have their own shape names, which fall through to n.h on both pages,
   and that is what keeps the label anchor and the occlusion silhouette on the
   same plane as the chart.

   ------------------------------------------------------------
   FOUR ROOFS

   Each of the four culls between the unfiltered and the filtered matrix makes
   a two-dimensional decision, and on the big map each was a 0.7-unit hatched
   box with that decision described in prose. Here there is room to draw it:
   ...
   horizontal roof of a building by one transform="matrix()".
   ...
   Load order on both pages: iso -> shapes -> culls-pop -> culls-draw -> data
   ============================================================ */

const CULLS_V = n => `var(--${n})`;
/* Every string on a roof is monospaced. A chart is a table of numbers with a
   picture on it, and a proportional face makes two ticks of the same decade
   different widths. */
...
/* ...AND THE SAME RULE AT ANY BUILDING SIZE.

   ANN_AT is in world-SVG pixels and was measured against a 4.2-unit roof,
   which is what /bioinformatics_pipe gives a cull because that page is only
   this row. On /pipeline the same cull is one object in one of four rows and
   is 1.5 units, so an offset of 236px put its annotation a building and a
   half away with a leader running off the bottom of the map.

   Everything about an annotation scales with its roof — the offset, the type,
   the leader, the terminal dot — so it is proportionally the same picture on
   either page rather than the same number of pixels. That is the rule the
   roof charts themselves already follow: chart space is CW x CW whatever the
   building's size, and there is not one screen-pixel literal in the roof code.
   ROOF_W is the size those numbers were measured at. */
const ROOF_W=4.2;
const annScale = n => Math.max(0.3, Math.min(1, (n.w||ROOF_W)/ROOF_W));
/* A NODE MAY NUDGE ITS OWN ANNOTATIONS, in world-SVG units, with annDx/annDy.
   The perpendicular rule puts them below the row, which on the page these were
...
function mkAnn(g,key,lines,tone,k){
  const col=tone||"var(--cull)", z=k||1;
  const NOHIT={"pointer-events":"none"};
  const line=g.appendChild(el("line",Object.assign({stroke:col,"stroke-width":(0.9*z).toFixed(2),
    "stroke-linecap":"round","stroke-opacity":"0"},NOHIT)));
  const dot=g.appendChild(el("circle",Object.assign({r:(2*z).toFixed(2),fill:col,"fill-opacity":"0"},NOHIT)));
  const t1=g.appendChild(el("text",Object.assign({"text-anchor":"start",fill:col,"font-size":12.5*z,
    "font-family":MONO,"font-weight":"600","letter-spacing":(1*z).toFixed(2),"fill-opacity":"0"},NOHIT)));
  lines.forEach((L,i)=>{ const sp=el("tspan",{x:0,dy:i===0?0:14*z}); sp.textContent=L; t1.appendChild(sp); });
...
function placeAnn(ann,target,bax,bay,alpha){
  ...
  /* every measurement here is against the annotation's OWN type size, which
     scales with its roof — see annScale. Hard-coding 12.5 put the leader's
     departure point a whole label away from the text on a small building. */
  const z=ann.z||1;
  const w=Math.max.apply(null,ann.lines.map(L=>L.length))*12.5*z*0.6;
  const left=ax, right=ax+w, top=ay-12.5*z, bot=ay+(ann.lines.length-1)*14*z+4*z, GAP=6*z;
  let sx,sy;
  ...
  ann.line.setAttribute("x2",(target[0]-ux*3.5*z).toFixed(1)); ann.line.setAttribute("y2",(target[1]-uy*3.5*z).toFixed(1));
  ...
  const bx=left-5*z, by=top-4*z, bw=w+10*z, bh=(bot-top)+8*z;
...
function drawDoubletRoof(g,n){
  ...
  const annS=mkAnn(g,n.id+":synth",["SYNTHETIC","REFERENCE"],"var(--accent)",annScale(n));
...
function drawComplexityRoof(g,n){
  ...
  const annU=mkAnn(g,n.id+":under",["UNDER-","AMPLIFIED"],null,annScale(n));
  const annO=mkAnn(g,n.id+":over", ["OVER-","AMPLIFIED"],null,annScale(n));
