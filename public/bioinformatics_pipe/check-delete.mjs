...
await p.route('**/api/bpipe_edits',r=>r.fulfill({status:200,contentType:'application/json',
...
if(await p.evaluate(()=>document.querySelector('#svg g[aria-label="Mito %"]').getAttribute('display'))!=='none')
...
if(await p.evaluate(()=>NODES.some(n=>n.id==='c3'))) fail('the deletion did not survive a reload');
if(await p.evaluate(()=>EDGES.some(e=>e.a==='c3'||e.b==='c3'))) fail('an edge to the deleted object survived');
