...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.68, lab:{dx:-1.4},
 ...
 does:"The read cycle drawn as the flow cell in its housing: a charcoal box with an elliptical glass chip recessed into its lid like a loaded slide, a plain grid etched across the glass and one small dim cluster in each of its cells. Four status lights on a touchscreen set into the front of the box, one in each base's colour, hold their colours and flare at random, never stopping. The clusters go off in a sweep from top to bottom like a stadium of flashbulbs, each in a base's colour and each a moment late on the sweep; then the field dims together and rests dark for a beat. The next cycle fires the same flashes in the same order, but every cluster in a different colour, because each cycle reads another base. A ribbed translucent cover, with a raised lip along its leading edge, runs out across the lid from its left edge to its right, rests there with the chip flashing through it, and draws back again. Against the back of the box stands a second module, as wide as the housing and half again as tall, with a touchscreen pad on its top; every edge of both boxes is rounded. On the box's right side a doorway stands open, and the reads leave through it as a single line of grey strands that flies five grid units out, dimensioned on the ground from the door, and gathers there into a slowly turning nebula of reads that swells as each strand joins it, a counter over it climbing towards the run's ~3 billion reads before a new run starts it empty.",
