...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.46, lab:{dx:-1.4},
 sub:"the flow cell · every cluster flashes, is imaged and goes dark together",
 does:"The read cycle drawn as the flow cell in its housing: a charcoal box with an elliptical glass chip recessed into its lid like a loaded slide, a plain grid etched across the glass and one small dim cluster in each of its cells. Four status lights along the front of the box, one in each base's colour, glow between cycles and flare with the field. Every cluster flashes to a colour at the same instant, holds, and a single bright line sweeps the chip once from top to bottom like a camera taking the whole surface in one pass; then the field dims together and rests dark for a beat. The next cycle lights the field again in a different pattern, because each cycle reads another base. Nothing travels across the surface and nothing happens in turn — the whole field acts as one.",
 built:"Nothing to cite. The requests that asked for this station described a picture — first a charcoal box with its top cut away and an arm reading a green pool, then a factory with a crane feeding a pool on its roof, then the flow cell alone with the whole field read at once, then that flow cell recessed into a charcoal housing with status lights on its front, then the cell made an elliptical chip with a plainer grid, a top-to-bottom scan and brighter lights — and named no instrument model, no chemistry, no cycle count and no read length, so this record names none either. What was actually sequenced, and on what, is S's record and stays there.",
