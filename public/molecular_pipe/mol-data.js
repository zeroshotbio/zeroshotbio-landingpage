...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.12, lab:{dx:-1.4},
 sub:"the flow cell · every cluster flashes, is imaged and goes dark together",
 does:"The read cycle drawn as the flow cell alone: a flat pane of glass with a fine grid across it and small dim clusters scattered evenly over the whole surface. Every cluster flashes to a colour at the same instant, holds, and a single bright line crosses the cell once like a camera taking the whole surface in one pass; then the field dims together and rests dark for a beat. The next cycle lights the field again in a different pattern, because each cycle reads another base. Nothing travels across the surface and nothing happens in turn — the whole field acts as one.",
 built:"Nothing to cite. The requests that asked for this station described a picture — first a charcoal box with its top cut away and an arm reading a green pool, then a factory with a crane feeding a pool on its roof, then the flow cell alone with the whole field read at once — and named no instrument model, no chemistry, no cycle count and no read length, so this record names none either. What was actually sequenced, and on what, is S's record and stays there.",
