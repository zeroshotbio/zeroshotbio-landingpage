...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.46, lab:{dx:-1.4},
 sub:"a factory · bases into the pool on the roof, grey reads out the door",
 does:"The read cycle drawn as a factory. The flow cell is a pool on the roof, with a bank of four vats either side of it — one per base, in S's four colours, the same four on each side. A gantry crane walks the roof on rails like one in a shipyard: it dips its grab into a vat, carries the base over the pool and lets it fall, and the clusters where it lands light up in that base's colour with a flash of it before settling. Each drop sends a short line of grey FASTQ files out of the door at the front, and they gather into a sphere that turns in front of the building until it is full.",
 built:"Nothing to cite. The requests that asked for this station described a picture — first a charcoal box with its top cut away and an arm reading a green pool, then a factory with the pool on its roof, vats of the four bases either side and a shipyard crane feeding it — and named no instrument model, no chemistry, no cycle count and no read length, so this record names none either. What was actually sequenced, and on what, is S's record and stays there.",
 cond:"Asked for from the page rather than read off an artefact, so what it carries is a figure and not a measurement. The cluster count, the crane's pace, how many files a drop sends out and how big the sphere grows are drawn, not read. A base dropped into the pool is the request's picture of a cycle adding a base and imaging it, not a claim about how any instrument delivers reagent or what any cluster read, and the files are not written here in the order or number any run writes them — turning the run into FASTQ is C4's step. It is a second view of S rather than a step of its own: nothing is made or consumed here, and nothing downstream depends on it."},
