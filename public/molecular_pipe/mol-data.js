...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.46, lab:{dx:-1.4},
 sub:"the lid off · an arm reads the flow cell as it passes",
 does:"The sequencer with its top cut away, so the flow cell can be watched while it is read. A green pool of clusters runs the length of the open box; an arm on a rail along the back rim slides back and forth over it, and the clusters directly beneath the arm flash bright as it passes and fade after it has moved on, so the light trails the arm like a scan line. The four small blocks beside the rail are S's four reagents, in S's colours.",
 ...
 cond:"Asked for from the page rather than read off an artefact, so what it carries is a figure and not a measurement. The cluster count, the arm's speed and how long a flash lingers are drawn, not read, and the arm reading the cell a strip at a time is the request's picture of a cycle being imaged rather than a claim about how any instrument scans or what any cluster read. It is a second view of S rather than a step of its own: nothing is made or consumed here, and nothing downstream depends on it."},
