/* ============================================================
   ...
   one for row 3. Nineteen stations: the thaw, three rounds of in-situ
   ...
   handoff where those reads change hands, and four stations that were
   asked for from the page rather than read off a protocol and say so on their
   own records — a library construct and a PCR amplification between the
   capture and the quantification, a pyramid, and the regrouping of reads into
   cells.

   THIRTEEN OF THE NINETEEN ARE LIFTED VERBATIM FROM pipeline-data.js.
   ...
   SIX ARE OWNED HERE — C4, C5, C6, C7, B8a AND B8′. They are the only stations
   ...
   station's does/built/cond may be edited in place. Four of them are the tail
   of the row; B8a and B8′ are not, and they sit between B8 and B9 under keys
   that are suffixes rather than a renumbering, so the thirteen lifted keys and
   the prose that cross-references them stay as they came.
...
/* NOT LIFTED, AND ASKED FOR FROM THE PAGE — the fourth of its kind here, and
   the second in the middle of the row. What the "Add a module" request
   described was a picture: three small bench objects — a strip of tubes in a
   low block, a thermal cycler, a violet 96-well plate with its left six columns
   filled — and over them one magnification in which a barcoded strand is cut
   in three places, Y-shaped adapters close onto the free ends, and the piece
   still carrying the barcodes assembles into the full construct, P5 to P7.

   IT NAMED NO PROTOCOL. No section, no reagent, no cycle count, no instrument
   model, so `built` names none and points at the request, the way B8a's does.
   What the record can say without inventing anything is where it stands
   against its own row: fragmentation, adapter ligation and the indexing PCR
   are already C1's and C2's, cited to sections 3.1 to 3.5, and both come after
   B8a and B9. `cond` says that rather than resolving it. In UNVERIFIED, and
   the badge says so.

   THE KEY IS A SUFFIX, AND THE OBVIOUS ONE WAS TAKEN. It lands between B8 and
   B8a, and "B8a" is already the amplification's, so it takes B8′ — B8 prime.
   Nothing after it is renumbered: B8a's key is cross-referenced in the header,
   the overview and UNVERIFIED, and the thirteen lifted keys downstream of it
   are part of what is lifted. */
{id:"LPR", key:"B8′", group:"cDNA capture and amplification", shape:"libraryprep", name:"Turn the cDNA into a library", x:12.97, y:R2, lane:"r2", w:0.72, d:0.72, h:0.4,
 sub:"cut · adapters · P5 to P7",
 does:"Barcoded cDNA is turned into a sequencing library. A long barcoded strand is cut in three places, adapters close onto the free ends, and the fragment still carrying the barcode chain becomes the complete construct — P5, a UDI, Read 1, the cDNA insert, the three barcodes, Read 2, a second UDI and P7.",
...
{id:"AMP", key:"B8a", group:"cDNA capture and amplification", shape:"pcramplify", name:"PCR amplification", x:13.35, y:R2, lane:"r2", w:0.72, d:0.72, h:0.4,
 ...
 does:"A polymerase binds each barcoded molecule in a closed block, runs the length of it and leaves a copy behind, and the glass fills. The three chips each strand carries are copied with it, which is why an amplified library still knows which cell each molecule came from. This is bulk PCR on free DNA in a tube — the cells were lysed at B7 and there is nothing in here holding anything — so it is not the in-cell chemistry the three barcoding rounds draw, and there is deliberately no plate and no well under this glass.",
...
{id:"SZD", key:"B9a", group:"cDNA capture and amplification", shape:"sizerun", name:"Measure the size distribution", x:14.85, y:R2, lane:"r2", w:0.95, d:0.95, h:0.4,
 ...
 does:"The amplified DNA is measured for how its fragment sizes are spread. A drop from each PCR tube goes into the well of its own lane on a cassette, two electrodes come down across every lane, the current crosses the plate from − to +, and the fragments run along them, the small ones faster than the large; what comes off is intensity against size, one trace per lane, written as the run goes. Nothing is made here — the DNA is only measured, and leaves the station as it arrived.",
...
{id:"RCY", key:"Sa", group:"The sequencer", shape:"readcycle", name:"The read cycle, cut away", x:21.75, y:R2, lane:"r2", w:1.60, d:1.30, h:0.68, lab:{dx:-1.4},
 sub:"the flow cell · clusters flash in a sweep across it, then go dark together",
 does:"The read cycle drawn as the flow cell in its housing: a charcoal box with an elliptical glass chip recessed into its lid like a loaded slide, a plain grid etched across the glass and one small dim cluster in each of its cells. A blank touchscreen is set into the front of the box. The clusters go off in a sweep from top to bottom like a stadium of flashbulbs, each in a base's colour and each a moment late on the sweep; then the field dims together and rests dark for a beat. The next cycle fires the same flashes in the same order, but every cluster in a different colour, because each cycle reads another base. A ribbed translucent cover, with a raised lip along its leading edge, runs out across the lid from its left edge to its right, rests there while the chip reads through it for as long as the cloud of reads takes to grow full, and draws back again; nothing lights while it moves or stands open. Against the back of the box stands a second module, as wide as the housing and half again as tall, with a raised touchscreen pad on its top carrying four status lights, one in each base's colour, lettered, which flash with the chip's field on every round; every edge of both boxes is rounded. On the box's right side a doorway stands open, and the reads leave through it as a single line of grey strands that flies five grid units out, dimensioned on the ground from the door, and gathers there into a slowly turning nebula of reads that swells as each strand joins it, a counter over it climbing towards the run's ~3 billion reads before a new run starts it empty.",
...
/* NOT LIFTED, AND ASKED FOR FROM THE PAGE — the second of its kind here, after
   the pyramid. What the "Add a module" request described was a FIGURE, at
   length: a plain compute block, a stream of reads arriving along a connector,
   four coloured chips on every read, the matching ones settling into clusters,
   and a cell outline fading in over each finished cluster. It also said what
   the figure is FOR, and that part is the record below: the cell was destroyed
   at B7 and comes back here out of the barcode combination, because nothing
   physical survived and the combination did.

   WHAT IT DOES NOT SAY is how any of that was done on this instance — no tool,
   no version, no threshold, no count — so neither does this record. built
   points at the request, the way C6's does, because the request is the only
   source there is; inventing a manual section for it would be inventing the
   manual. It is in UNVERIFIED for that reason and it says so on the badge.

   IT IS WIDER THAN C4, C5 AND C6. Those three share a .95 tile and this one
   does not, because six clusters of seven reads each carrying four legible
   chips does not fit on it. The 1.30 was paid for at the end of the lane — see
   the note above LANES — so no gap already on the row moved. */
{id:"RGP", key:"C7", group:"The sequencer", shape:"regroup", name:"Group the reads back into cells", x:25.0, y:R2, lane:"r2", w:1.30, d:1.30, h:0.40,
 sub:"four chips per read · one combination, one cell",
 does:"Reads arrive with no order to them, and every one carries the four barcodes it was given — the three ligated inside the cell and the fourth added by PCR to the tube. Reads carrying the same four are one cell's reads, so they gather; reads carrying different ones pull away. Each group that comes out whole is drawn back as a cell. The cell itself was destroyed at B7 and nothing physical is left of it: what survived is the combination, written into every one of its molecules across four rounds, and that is what is read back here.",
 built:"Nothing to cite. The request that asked for this station described a picture and said what the picture is for; it named no software, no version, no threshold and no count, so this record names none either. What it stands on is already on the row and belongs to the stations that own it — B7 for the moment the cell stops being a cell, B2, B4 and B6 for the three barcodes ligated into it, and C2 for the fourth, which is an index on the tube rather than on the cell.",
 cond:"Asked for from the page rather than read off an artefact, so what it carries is a figure and not a measurement. The count on the label is the number of clusters standing in the frame and nothing else — how many cells this run actually recovered is the matrix's to say, and no number off this instance was put on this drawing. The six complete clusters are a drawing's convenience too: a combination carried by one read cannot be shown to gather, so the stream shows a few combinations dealt into each other rather than a distinct one per read."},
...
const EDGES = [
  ...
  {a:"SB",b:"CAP",kind:"lib"},
  /* B8a was inserted between these two, so the one track that ran CAP -> QCD
     is now two. Same `lib` kind on both: what is running down this stretch is
     the same amplified cDNA it always was. */
  ...
  /* C7 was asked for as the END of the row and the row already ran one station
     past the handoff, so the track that reaches it comes off the pyramid rather
     than off C5. Same `read` kind: what is running down this end of the row is
     still the sequencer's output. */
  {a:"PYR",b:"RGP",kind:"read"}
];
...
/* ONE LANE, AND IT IS /pipeline's OWN — plus exactly the room C4 and C5 cost.
   ...
   C7, the regrouping, cost 1.814: 1.30 of station — it is wider than the three
   before it because six clusters of seven reads will not fit on their tile —
   plus 0.6 of minor gap at the same k 0.8568, x1 27.624 -> 29.438, priced the
   same way, k unmoved again and all fifteen existing gaps where they were.
   ...
   B8a, THE PCR AMPLIFICATION, IS THE FIRST TO BE PAID FOR IN THE MIDDLE OF THE
   ROW rather than at its end, and it costs the same way and for the same
   reason: 1.234, being 0.72 of station plus one extra minor gap of 0.6 at the
   same k 0.8567, x1 29.438 -> 30.672, priced by scripts/pipeline_lane.mjs. The
   arithmetic is the tool's and not a sum done by hand — one gap becomes two, so
   what is bought is the station's width plus the difference between the gaps,
   and the tool reports k unmoved and all sixteen existing gaps landing exactly
   where they already were. The nine stations downstream of B8 slide 1.234 along
   and nothing else on the row moves.

   B8′, THE LIBRARY CONSTRUCT, went into the same gap one step earlier and cost
