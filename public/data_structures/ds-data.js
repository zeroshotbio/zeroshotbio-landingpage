...
{id:"BRONZE", key:"1", group:"① Bronze", groupMark:true, anchor:true,
 ...
 built:"Four datasets, 1,258 objects, 7,730,616,859,647 bytes read on 2026-08-29, unchanged to the byte across five reads. Every object is in the STANDARD storage class — there is no Glacier or Intelligent-Tiering anywhere in the bucket, which for 7 TiB of write-once archive is a standing monthly cost and a decision nobody has recorded making. The tiles are drawn by area: MegaFin's three deliveries are 92.2% of the tier between them, and MiniFin — the dataset the entire pipeline has been built and proven against — is the 7.8% tile.",
 ...
 kv:[["Bucket","zsb-bronze-fortknox"],["Objects","1,258"],["Size","7.03 TiB (7,730,616,859,647 B)"],["Storage class","STANDARD, all 1,258"],["Versioned","yes — every object carries a VersionId"],["Encrypted","AES256 at rest"],["Written by","humans only"],["Read on","2026-08-29 — unchanged, five reads"]]},
...
{id:"SILVER", key:"4", group:"④ Silver", groupMark:true, anchor:true,
 ...
 built:"86 objects, 21,276,994,543 bytes, read 2026-08-29 — four objects and 2.91 GiB more than the last read, all of it the v2 and v3 releases. It is 0.28% the size of bronze, which is the medallion architecture working exactly as intended: 7 TiB of vendor deliverables reduce to 20 GiB of counts.",
 cond:"<mark>The tile this map was drawn to complain about is now the one doing the work.</mark> <mark>minifin/</mark> holds thirteen objects: the six flat keys the previous pipeline left, the ledger, and three version prefixes of two objects each. Those releases are 98% of the tile's bytes. The old six are still there beside them, unversioned, provenance in sidecars — a released prefix does not tidy up what preceded it, and nothing on this map proposes deleting them. The other two tiles have not moved a byte in five reads and were written by neither repo here. <mark>zebrahub/</mark> (10.4 GiB, 12 objects, 2026-07-27) predates the zsb-* repos and is the placeholder both zsb-silver and zsb-gold deleted from their own source trees as 'not the shape the real transforms take'. <mark>megafin-1/characterization/</mark> (5.0 GiB, 61 objects) is an analysis working set — scripts, CSVs, diff reports and a 4.9 GiB h5ad — not a published artifact, and it has no version prefix. The tier still reads two ways at once and that is still the honest picture, but the balance has tipped: <mark>the bytes written under the convention now outweigh the bytes that predate it</mark> in the only tile either repo touches. The sentence this note carried for two reads — <em>silver is full of things and empty of releases</em> — was half retired on the third read. It is now retired outright, and the half that survived it was <em>full of things</em>, which is also still true.",
...
const SNIPPETS = {
  BRONZE: () => ({
    ...
    note: "Read from the live bucket on 2026-08-29. Identical to all four earlier reads, to the byte."
  }),
  BFETCH: () => ({
