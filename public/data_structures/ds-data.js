/* ============================================================
   ...
   all eight matched size and etag, for the third read running, and again on
   2026-08-26 for the fourth. Nothing on either side has moved since; see the
   fourth-read note below for the two pieces of prose that had.
...
{id:"SREPO", key:"5", group:"⑤ Silver → Gold", groupMark:true, anchor:true,
 ...
 cond:"<mark>The gate this map has complained about twice is gone, and it was closed the way it was always going to be: by somebody writing down the key.</mark> The reading side used to be blocked on a convention the writing side had already chosen; that reconciliation happened, and what came out is not a convention but a named object with a pinned identity. The two remaining stubs are still <em>conventions and sign-offs rather than algorithms</em> — the Gold v1 QC sign-off, and the gold object-key format — and no amount of tooling closes either. This repo also carries <mark>every open pull request in the set</mark>, and there are two of them: <mark>#17, porting Trailmaker QC steps 3 and 4 and naming the parse recipe</mark> — the first work anywhere on this map aimed at the sign-off rather than around it — and #18, which is documentation. The other three repos have none open. Worth watching on the next read.",
 kv:[["Repo","zeroshotbio/zsb-silver"],["HEAD","4a64566 (main)"],["Commits","34, since 2026-07-22"],["Source","870 LOC · 366 in minifin/"],["Tests","6 files"],["Steps written","1 of 3 — fetch"],["Depends on","zsb-medallion @ v0.8.0"],["Open PRs","2 — #17 Trailmaker QC 3 and 4, #18 docs"],["Bytes moved","0 — the fetch has not been run from here"]]},
...
<p><mark>How to read the conduits.</mark> A solid line with dots moving along it has carried bytes and can be shown to have. A dashed line is code that exists and has never run. Two of the four are solid, and the pair of them is one dataset going down and coming back out again.</p>`,
  built: `<p>Everything here was read on <mark>2026-08-24</mark> from the live AWS account and from the four repositories at their current main commits — zsb-medallion 00b71e8, zsb-bronze 6126161, zsb-silver 4a64566, zsb-gold 513ca22. This is the third read of this map. The first two found the buckets byte-for-byte identical and all the movement in the repo column; this one is the other way round — the left column changed, and it changed because a step on the right finally ran. Re-read on 2026-08-26, the fourth: <mark>both sides came back identical</mark> — the same object counts and byte totals in bronze and silver, the same three prefixes at the same sizes, gold still refusing both calls, and all four repos on the commits above with nothing merged in two days. The eight pins and the release object matched again. No figure on this map changed; two sentences did, and both were claims that had outlived the read they were made in.</p>
...
<p class="cond">The two that remain are both in the last two steps and neither is a research problem. The <mark>gold v1 QC sign-off</mark> is a question for a person, and there is now, for the first time, work aimed at it rather than around it: zsb-silver's open PR #17 ports Trailmaker QC steps 3 and 4 and names the parse recipe, and it was still open on 2026-08-26 — now beside a second, documentation-only one. Every open pull request in this set is in that one repo, which is where the remaining work is. The <mark>gold object-key convention</mark> blocks two repos from opposite directions at once, and it is the same question the silver key answered one tier up — the worked example is sitting there.</p>
